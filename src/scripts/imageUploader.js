export const imageUploader = (selector = '.js-upload-img') => {
  const containers = document.querySelectorAll(selector)

  containers.forEach((container) => {
    const input = container.querySelector('input[type=file]')
    let list = container.querySelector('.image-uploader__list')

    const handleRemove = (e) => {
      e.preventDefault()
      const removeBtn = e.target.closest('.image-uploader__remove-btn')
      if (removeBtn) {
    ...
    const handleFiles = (files) => {
      ...
      if (!list) {
        ...
        list.classList.add('image-uploader__list')
      ...
      Array.from(files).forEach((file) => {
        ...
        imgContainer.classList.add('image-uploader__preview-img')
        ...
        removeBtn.classList.add('image-uploader__remove-btn')
        removeBtn.innerHTML = `
