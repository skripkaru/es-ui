import Toastify from 'toastify-js'
import 'toastify-js/src/toastify.css'

export const initMessage = () => {
  const triggers = document.querySelectorAll('.js-message-trigger')

  const showMessage = (messageId) => {
    const message = document.querySelector(`[data-message='${messageId}']`)
    if (message) {
      Toastify({
        node: message,
        duration: 3000,
        close: false,
        gravity: 'bottom',
        position: 'right',
        stopOnFocus: true,
        className: 'message'
      }).showToast()
    }
  }

  triggers.forEach((trigger) => {
    trigger.addEventListener('click', () => {
      const messageId = trigger.dataset.messageTrigger
      showMessage(messageId)
    })
  })
}

export const createComponent = () => {
  const component = document.createElement('div')

  const wrapper = document.createElement('div')
  wrapper.hidden = true

  const message = document.createElement('div')
  message.classList.add('message')
  message.dataset.message = 'message'
  message.innerHTML = `
    <div class="message__inner">
      <span>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Impedit, repellendus.</span>
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
        <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0"/>
      </svg>
    </div>
  `

  const button = document.createElement('div')
  button.classList.add('button', 'button--primary', 'js-message-trigger')
  button.textContent = 'Create message'
  button.dataset.messageTrigger = 'message'

  wrapper.appendChild(message)
  component.appendChild(wrapper)
  component.appendChild(button)

  return component
}
